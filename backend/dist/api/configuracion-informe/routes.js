...
router.get('/', async (req, res) => {
    try {
        ...
        let config = await model_1.ConfiguracionInforme.findOne({
            usuarioId: usuarioId.toString(),
            contratoId: contratoId.toString()
        });
        // Si no existe, crear configuración por defecto
        if (!config) {
            config = new model_1.ConfiguracionInforme({
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const config = await model_1.ConfiguracionInforme.findOneAndUpdate({ id }, { ...req.body, updatedAt: new Date() }, { new: true });
        res.json(config);
    }
    catch (error) {
