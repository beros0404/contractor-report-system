...
const model_2 = require("../usuarios/model");
...
async function getDriveClient(usuarioId) {
    ...
    if (usuario.googleTokens.expiry_date &&
        ...
        try {
            ...
            await model_2.Usuario.findOneAndUpdate({ supabaseId: usuarioId }, { googleTokens: credentials });
...
async function crearEstructuraCarpetas(drive, usuarioId, contratoId, actividadId) {
    try {
...
router.get('/', async (req, res) => {
    try {
        ...
        const evidencias = await model_1.Evidencia.find({
...
router.get('/descargar/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { usuarioId } = req.query;
        const evidencia = await model_1.Evidencia.findOne({
            id: id,
            usuarioId: usuarioId?.toString()
        });
        if (!evidencia) {
...
router.post('/upload', upload.single('archivo'), async (req, res) => {
    try {
        ...
        const nuevaEvidencia = new model_1.Evidencia({
            ...
            try {
                ...
                if (carpetas) {
                    ...
                    const media = {
                        ...
                        body: stream_1.Readable.from(urlBuffer)
                    };
                    const file = await drive.files.create({
                        requestBody: fileMetadata,
                        media: media,
        ...
        if (drive) {
            try {
                ...
                if (carpetas) {
                    ...
                    const media = {
                        mimeType: 'text/plain',
                        body: stream_1.Readable.from(notaBuffer)
                    };
                    const file = await drive.files.create({
                        requestBody: fileMetadata,
                        media: media,
